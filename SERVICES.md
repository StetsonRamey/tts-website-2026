...
- Lead source labeling: the exact printed van-QR combination (`utm_source=van`, `utm_medium=qr`, `utm_campaign=van_wrap` carried in the session attribution) maps the `Which Form` field to the literal option `QR Code` (`server.go` `leadSource`/`isVanQR`). The original UTMs remain in the lead comments via the attribution summary, so van leads stay distinguishable from future QR campaigns. Missing or mismatched tags never produce `QR Code`; ordinary, Google Ads, Meta, and non-van free-estimate leads keep their existing values.
