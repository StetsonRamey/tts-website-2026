...
- Customer payments are manual/customer-initiated. Keep Checkout in one-time `mode=payment` without off-session setup, and invoices at `collection_method=send_invoice` with `auto_advance=false`. Signup/reminder delivery must not charge saved cards. Do not introduce subscriptions or automatic renewals without an explicit business-policy change. For unexpected-charge reports, first verify that the PaymentIntent's actual Stripe customer matches the reported Airtable record; follow the read-only investigation procedure in `SERVICES.md` before changing billing or paid status.
- The payment webhook currently maps Stripe customer ID to the first matching Master Customers row. Audit shared Stripe IDs as well as duplicate Stripe profiles: several Airtable properties using one Stripe ID can cause the wrong property to be marked paid. See `SERVICES.md` for the read-only duplicate/contact audit procedure. Shared emails alone are not grounds to merge/delete customers, and customer exports/reports must stay outside Git and the public web root.
...
- The paid Google Ads landing page is `/free-estimate/`. It uses the same `/contact` form handler and Google Ads conversion event as `/contact/`, and saves allow-listed Google click IDs/UTMs in the lead's `Tracking Metrics` field for later quality reconciliation. A browser conversion may fire only after `/contact` returns `X-Lead-Saved: true`, which is sent after Airtable confirms creation. Keep it `noindex` unless its purpose is intentionally expanded to organic search.
- Lead source: the `Which Form` single-select field on the 2026 Leads table carries the lead source. `leadSource`/`isVanQR` in `server.go` map the exact printed van-QR UTM combination (`utm_source=van`, `utm_medium=qr`, `utm_campaign=van_wrap`) to the literal option `QR Code`; other submissions keep `whichForm` behavior (`Main Contact` or `/free-estimate/ ads lander`). Original UTMs are retained in the lead's `Tracking Metrics` field via `formatAttribution`. The `QR Code` option must exist on the field (currently: `Main Contact`, `Mailer`, `/free-estimate/ ads lander`, `QR Code`) — do not invent new select options in code.
- Meta tracking: the Pixel (`params.metaPixelId` in `hugo.toml`) fires PageView on public marketing pages only; opt a page out with `tracking.metaPixel: false` front matter. `/contact` mints one `tts-lead-…` event ID per accepted lead, returns it as `X-Meta-Event-Id`, sends a server-side CAPI `Lead` with it after the Airtable save, and stores it in the lead's `Tracking Metrics` field (`fldqlM7utGqqVnVa7`). The browser Lead must reuse that ID. Never fire a Lead from `/thank-you/`, never send inquiry text/addresses to Meta, never synthesize click IDs, and keep `META_CAPI_ACCESS_TOKEN` out of Hugo assets and logs. Session attribution in `assets/js/attribution.js` keeps an in-memory current-page snapshot and uses `sessionStorage` only for cross-page handoff; a URL with campaign parameters replaces (never merges) prior attribution. If browser storage is unavailable, do not claim attribution persists after navigation. Preserve opaque click/browser identifiers in full; only human-readable UTM/path CRM values are capped. The Airtable `Comments` field carries only the customer's own form notes — all attribution/UTM/Meta identifiers go to `Tracking Metrics`, never `Comments`.
